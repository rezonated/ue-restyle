...
{
	using KeyInitType = TTypeTraits<TSharedPtr<IRestyleThemeInterface>>::ConstPointerType;
	using ElementInitType = TCallTraits<TSharedPtr<IRestyleThemeInterface>>::ParamType;

	/**
	 * @return The key used to index the given element.
	 */
	static FORCEINLINE KeyInitType GetSetKey(ElementInitType Element) { return Element; }

	/**
	 * @return True if the keys match.
	 */
	static FORCEINLINE bool Matches(KeyInitType A, KeyInitType B) { return A->GetId() == B->GetId(); }
...
public:
	...
	static FRestyleModule* Get() { return static_cast<FRestyleModule*>(FModuleManager::Get().GetModule("Restyle")); }
