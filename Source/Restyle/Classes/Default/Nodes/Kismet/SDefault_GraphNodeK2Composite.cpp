...
void SDefault_GraphNodeK2Composite::Construct(const FArguments& InArgs, UK2Node_Composite* InNode)
{
	...
	CachedOutlineWidth = UDefaultThemeSettings::GetOutlineWidth(
		NodeRestyleSettings->CollapsedNode.GetTypeData(GetCollapsedNodeType()).GetState(CachedState).Body.Get().OutlineWidth
	);
...
EDTCollapsedNodeType SDefault_GraphNodeK2Composite::GetCollapsedNodeType() const { return EDTCollapsedNodeType::Default; }
						...
						[
							...
							[
								SAssignNew(InlineEditableText, SInlineEditableTextBlock)
								                                                        .Style(FAppStyle::Get(),
								                                                               FNodeRestyleStyles::CollapsedNode_Title_MainText)
							//.Text(NodeTitle.Get(), &sSDefault_NodeTitle::GetHeadTitle)
								                                                        .Text(NodeTitle->GetHeadTitle())
								                                                        .OnVerifyTextChanged(
									                                                        this,
									                                                        &SDefault_GraphNodeK2Composite::OnVerifyNameTextChanged)
								                                                        .OnTextCommitted(
									                                                        this,
									                                                        &SDefault_GraphNodeK2Composite::OnNameTextCommited)
								                                                        .IsReadOnly(
									                                                        this, &SDefault_GraphNodeK2Composite::IsNameReadOnly)
								                                                        .IsSelected(
									                                                        this,
									                                                        &SDefault_GraphNodeK2Composite::IsSelectedExclusively)
...
TSharedPtr<SToolTip> SDefault_GraphNodeK2Composite::GetComplexTooltip()
{
	if (UEdGraph* BoundGraph = GetInnerGraph()) {
		...
		TSharedRef<IDocumentationPage> DocPage = IDocumentation::Get()->
			GetPage(GraphNode->GetDocumentationLink(), nullptr);
...
FText SDefault_GraphNodeK2Composite::GetTooltipTextForNode() const { return GraphNode->GetTooltipText(); }
