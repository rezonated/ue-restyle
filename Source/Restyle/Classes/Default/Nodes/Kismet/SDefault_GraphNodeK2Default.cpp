...
void SDefault_GraphNodeK2Default::UpdateCompactNode_New()
{
	...
	auto AddIdentificationContent = [this, &NodeOverlay, &State, &TitleType, &CompactNode, &AlternativeTitleWidget]()
	{
		...
		if (!Image.IsValid()) {
			...
			Image = SNew(SImage).Image(FAppStyle::GetBrush(Id)).ColorAndOpacity(IdColor);
		}
	...
	const UNodeRestyleSettings* NodeRestyleSettings = UNodeRestyleSettings::Get();
	auto AddComment = [this, NodeRestyleSettings]()
	{
	...
	// Hide pin labels
	for (auto InputPin : this->InputPins) { if (InputPin->GetPinObj()->ParentPin == nullptr) { InputPin->SetShowLabel(false); } }

	for (auto OutputPin : this->OutputPins) { if (OutputPin->GetPinObj()->ParentPin == nullptr) { OutputPin->SetShowLabel(false); } }
...
void SDefault_GraphNodeK2Default::UpdateGraphNode()
{
	...
	if (K2Node->ShouldDrawCompact()) { UpdateCompactNode_New(); }
	else { SDefault_GraphNodeK2Base::UpdateGraphNode(); }
}
...
void SDefault_GraphNodeK2Default::OnStateUpdated(EDTGraphNodeState NewState)
{
	...
	if (bCompactMode) {
		const UNodeRestyleSettings* NodeRestyleSettings = UNodeRestyleSettings::Get();
		const auto& State = NodeRestyleSettings->CompactNode.States.GetState(CachedState);
		if (CompactTitleTextBlock.IsValid()) { CompactTitleTextBlock->SetColorAndOpacity(State.TitleColor.Get()); }
		...
		EDTGraphNodeTitleType TitleType = GetNodeType();
		const UNodeRestyleSettings* NodeRestyleSettings = UNodeRestyleSettings::Get();
		const auto& State = NodeRestyleSettings->Node.GetTypeData(TitleType).GetState(CachedState);
