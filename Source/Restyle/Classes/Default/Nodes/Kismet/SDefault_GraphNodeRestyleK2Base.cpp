...
TSharedPtr<SWidget> SDefault_GraphNodeRestyleK2Base::CreateEnabledStateWidget()
{
	if ((GraphNode->GetDesiredEnabledState() != ENodeEnabledState::Enabled) && !GraphNode->
		...
		EDTEnabledStateType Type = bDevelopmentOnly ? EDTEnabledStateType::Development : EDTEnabledStateType::Disabled;
		const FDTEnabledState& State = UNodeRestyleSettings::Get()->StateWidget.Types[static_cast<int>(Type)];
		return
				SNew(SBorder)
				...
				[
					SNew(STextBlock)
					.Text(FText::FromString(State.Title))
...
{
	const bool bHidePins = OwnerGraphPanelPtr.IsValid() && (OwnerGraphPanelPtr.Pin()->GetPinVisibility() !=
...
void SDefault_GraphNodeRestyleK2Base::UpdateGraphNode()
{
	...
	EDTGraphNodeState CurrentState = LastUpdatedState;
	const UNodeRestyleSettings* NodeRestyleSettings = UNodeRestyleSettings::Get();
	const auto& Node = NodeRestyleSettings->Node;
	...
	auto MakeDefaultTitleAreaWidget = [this, &Node, &State, CurrentState, TitleType]()-> TSharedRef<SOverlay>
	{
		auto GetIcon = [this]() -> const FSlateBrush* {
			...
			if (GraphNode != nullptr && GraphNode->ShowPaletteIconOnNode()) { return GraphNode->GetIconAndTint(IconColor).GetOptionalIcon(); }
			return nullptr;
		};
	...
	auto UpdateTooltip = [this]()
	{
		if (!SWidget::GetToolTip().IsValid()) {
			TSharedRef<SToolTip> DefaultToolTip = IDocumentation::Get()->CreateToolTip(
				TAttribute<FText>(this, &SDefault_GraphNodeRestyleK2Base::GetNodeTooltip), nullptr,
	...
	auto AddEnabledStateWidget = [this, &InnerVerticalBox, NodeRestyleSettings]
	{
	...
	auto AddVerticalBox = [this, &MainVerticalBox, &TagMeta, &InnerVerticalBox, &NodeRestyleSettings, &State, &TitleType,
			&CurrentState]
	{
	...
	auto AddCommentIfNeeded = [this, NodeRestyleSettings]()
	{
		bool SupportsBubble = true;
		if (GraphNode != nullptr) { SupportsBubble = GraphNode->SupportsCommentBubble(); }
...
{
	const UNodeRestyleSettings* NodeRestyleSettings = UNodeRestyleSettings::Get();
	float ContentSpacing = UDefaultThemeSettings::GetSpacing(NodeRestyleSettings->Node.ContentSpacing);
	return SNew(SBorder)
			.BorderImage(FAppStyle::GetBrush("NoBorder"))
			.HAlign(HAlign_Fill)
			.VAlign(VAlign_Fill)
...
void SDefault_GraphNodeRestyleK2Base::SetOwner(const TSharedRef<SGraphPanel>& OwnerPanel)
{
	...
	if (UNodeRestyleSettings::Get()->bNewZoom) { OwnerPanel->SetZoomLevelsContainer<FRestyleZoomLevelsContainer>(); }
}

void SDefault_GraphNodeRestyleK2Base::SetupRenderOpacity() { if (IsDisabled()) SetRenderOpacity(UNodeRestyleSettings::Get()->DisabledOpacity); }
                                                                             ...
                                                                             TSharedPtr<SToolTip> CustomTooltip)
{
	...
	const UPinRestyleSettings* PinRestyleSettings = UPinRestyleSettings::Get();
	const FDTPinSequenceInput& Seq = PinRestyleSettings->Inputs.Sequence;
	...
	if (CustomTooltip.IsValid()) { Tooltip = CustomTooltip; }
		...
		Tooltip = IDocumentation::Get()->CreateToolTip(PinTooltipText, nullptr, GraphNode->GetDocumentationLink(),
	...
	TSharedRef<SButton> AddPinButton = SNew(SButton)
		...
		[
			SNew(SBox)
			.MinDesiredHeight(PinRestyleSettings->Base.MinDesiredHeight)
			.VAlign(VAlign_Center)
			[
...
void SDefault_GraphNodeRestyleK2Base::UpdateErrorInfo_New()
{
	...
	if (GraphNode->bHasCompilerMessage) {
		if (GraphNode->ErrorType <= EMessageSeverity::Error) { ErrorInfoType = EDTNodeReportType::Error; }
		else if (GraphNode->ErrorType <= EMessageSeverity::Warning) { ErrorInfoType = EDTNodeReportType::Warning; }
		else { ErrorInfoType = EDTNodeReportType::Info; }
	}
	else if (!GraphNode->NodeUpgradeMessage.IsEmpty()) { ErrorInfoType = EDTNodeReportType::Upgrade; }

	if (ErrorInfoType != EDTNodeReportType::Num) { ErrorMsg = UNodeRestyleSettings::Get()->NodeReports[static_cast<int>(ErrorInfoType)].Title; }
	else {
		ErrorMsg.Empty();
...
bool SDefault_GraphNodeRestyleK2Base::IsInvalid() const { return GraphNode->IsDeprecated() || GraphNode->IsNodeUnrelated(); }

bool SDefault_GraphNodeRestyleK2Base::IsDisabled() const { return !GraphNode->IsNodeEnabled() || GraphNode->IsDisplayAsDisabledForced(); }
...
void SDefault_GraphNodeRestyleK2Base::OnStateUpdated(EDTGraphNodeState NewState)
{
	EDTGraphNodeTitleType TitleType = GetNodeType();
	const UNodeRestyleSettings* NodeRestyleSettings = UNodeRestyleSettings::Get();
	const FDTNodeStateData& State = NodeRestyleSettings->Node.GetTypeData(TitleType).GetState(CachedState);
	...
	if (TitleIcon.IsValid()) { TitleIcon->SetColorAndOpacity(State.Title.Icon.Get()); }
	...
	if (InlineEditableText.IsValid()) { InlineEditableText->SetColorAndOpacity(State.Title.MainText.Get()); }
	...
	if (EnabledStateWidgetBox.IsValid()) { EnabledStateWidgetBox->SetPadding(GetEnabledStateWidgetPadding()); }
}
