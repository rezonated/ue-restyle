...
TSharedPtr<SToolTip> SDefault_GraphNodeBlendSpaceGraph::GetComplexTooltip()
{
	if (UBlendSpaceGraph* BlendSpaceGraph = CastChecked<UBlendSpaceGraph>(GetInnerGraph())) {
		...
		if (DocPage->HasExcerpt(GraphNode->GetDocumentationExcerptName())) {
			...
			[
				IDocumentation::Get()->CreateToolTip(FText::FromString("Documentation"), nullptr, GraphNode->GetDocumentationLink(),
...
TSharedRef<SWidget> SDefault_GraphNodeBlendSpaceGraph::CreateNodeBody()
{
	...
	return SNew(SVerticalBox)
		...
		[
			SDefault_AnimationGraphNode::CreateNodeFunctionsWidgetNew(BlendSpaceNode, MakeAttributeLambda(UseLowDetailNode))
		]
		+ SVerticalBox::Slot()
		.AutoHeight()
		[
			...
			[
				...
				.OnGetBlendSpaceSampleName(FOnGetBlendSpaceSampleName::CreateLambda(
					[this, WeakBlendSpaceNode = TWeakObjectPtr<UAnimGraphNode_BlendSpaceGraphBase>(BlendSpaceNode)](int32 InSampleIndex) -> FName
					{
