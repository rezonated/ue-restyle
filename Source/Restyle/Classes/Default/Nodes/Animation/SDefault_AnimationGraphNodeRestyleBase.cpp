...
void SDefault_AnimationGraphNodeRestyleBase::UpdateGraphNode()
{
	...
	const FDTNode& Node = NodeRestyleSettings->Node;
	EDTGraphNodeTitleType TitleType = GetNodeType();
	const FDTNodeStateData& State = Node.GetTypeData(TitleType).GetState(CurrentState);
	{
		InputPins.Empty();
		OutputPins.Empty();
	...
	auto UpdateTooltip = [this]()
	{
		if (!SWidget::GetToolTip().IsValid()) {
			TSharedRef<SToolTip> DefaultToolTip = IDocumentation::Get()->CreateToolTip(
				TAttribute<FText>(this, &SDefault_AnimationGraphNodeRestyleBase::GetNodeTooltip), nullptr, GraphNode->GetDocumentationLink(),
...
TSharedRef<SWidget> SDefault_AnimationGraphNodeRestyleBase::CreateTitleWidget(TSharedPtr<SNodeTitle> InNodeTitle)
{
	...
	SAssignNew(InlineEditableText, SInlineEditableTextBlock)
		...
		//.Text(NodeTitle.Get(), &SDefault_NodeTitle::GetNodeTitle_New)
	                                                        ...
	                                                        .OnVerifyTextChanged(
		                                                        this, &SDefault_AnimationGraphNodeRestyleBase::OnVerifyNameTextChanged)
...
bool SDefault_AnimationGraphNodeRestyleBase::IsInvalid() const { return GraphNode->IsDeprecated() || GraphNode->IsNodeUnrelated(); }

bool SDefault_AnimationGraphNodeRestyleBase::IsDisabled() const { return !GraphNode->IsNodeEnabled() || GraphNode->IsDisplayAsDisabledForced(); }
