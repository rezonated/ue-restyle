...
void SDefault_AnimationGraphNode::Construct(const FArguments& InArgs, UAnimGraphNode_Base* InNode)
{
	...
	CachedOutlineWidth = UDefaultThemeSettings::GetOutlineWidth(
		NodeRestyleSettings->Node.GetTypeData(GetNodeType()).GetState(CachedState).Body.Get().OutlineWidth
	);
	EnabledStateWidgetAdditionalPadding = UDefaultThemeSettings::GetMargin(NodeRestyleSettings->StateWidget.Padding);
	SetCursor(EMouseCursor::CardinalCross);
	UpdateGraphNode();

	ReconfigurePinWidgetsForPropertyBindings(CastChecked<UAnimGraphNode_Base>(GraphNode), SharedThis(this),
	...
	access_private::IndicatorWidget(*this) =
		...
		.ToolTip(IDocumentation::Get()->CreateToolTip(
			LOCTEXT("AnimGraphNodeIndicatorTooltip", "Fast path enabled: This node is not using any Blueprint calls to update its data."),
			nullptr, TEXT("Shared/GraphNodes/Animation"), TEXT("GraphNode_FastPathInfo")))
...
void SDefault_AnimationGraphNode::HandleNodeTitleChangedNew() { if (TitleExtraText.IsValid()) { TitleExtraText->MarkDirty(); } }
...
void SDefault_AnimationGraphNode::CreateBelowPinControls(TSharedPtr<SVerticalBox> MainBox)
{
	if (UAnimGraphNode_Base* AnimNode = CastChecked<UAnimGraphNode_Base>(GraphNode, ECastCheckedType::NullAllowed)) {
		auto UseLowDetailNode = [this]() { return GetCurrentLOD() <= EGraphRenderingLOD::LowDetail; };
...
{
	if (access_private::LastHighDetailSize(*this).IsNearlyZero()) { return false; }

	if (const SGraphPanel* MyOwnerPanel = GetOwnerPanel().Get()) { return (MyOwnerPanel->GetCurrentLOD() <= EGraphRenderingLOD::LowestDetail); }
	return false;
}

FVector2D SDefault_AnimationGraphNode::GetLowDetailDesiredSize() const { return access_private::LastHighDetailSize(*this); }
	...
	void Construct(const FArguments& InArgs, UAnimGraphNode_Base* InNode)
	{
		...
		PropertyRowGenerator->RegisterInstancedCustomPropertyTypeLayout(FMemberReference::StaticStruct()->GetFName(),
		                                                                FOnGetPropertyTypeCustomizationInstance::CreateStatic(
			                                                                &FBlueprintMemberReferenceDetails::MakeInstance));
		...
		auto AddFunctionBindingWidget = [this, InNode, &Box, &VerticalBox, &AnimationStyle](FName InCategory, FName InMemberName)
		{
			...
			for (const TSharedRef<IDetailTreeNode>& RootTreeNode : PropertyRowGenerator->GetRootTreeNodes()) {
				if (RootTreeNode->GetNodeName() == InCategory) {
					...
					for (int32 ChildIdx = 0; ChildIdx < Children.Num(); ChildIdx++) {
						...
						if (ChildPropertyHandle.IsValid() && ChildPropertyHandle->GetProperty() && ChildPropertyHandle->GetProperty()->GetFName()
							== InMemberName) {
							DetailTreeNode = Children[ChildIdx];
							PropertyHandle = ChildPropertyHandle;
							PropertyHandle->SetOnPropertyValueChanged(FSimpleDelegate::CreateLambda([InNode]() { InNode->ReconstructNode(); }));
							break;
						}
					}
				}
			...
			if (DetailTreeNode.IsValid() && PropertyHandle.IsValid()) {
				...
				catch (...) { checkf(false, L"SPropertyNameWidget's structure has been changed, code needs to be updated"); }
				...
				catch (...) { checkf(false, L"SPropertyBinding's structure has been changed, code needs to be updated"); }
				const FButtonStyle* ButtonStyle = bPinsRegistered
					                                  ? &FAppStyle::Get().GetWidgetStyle<FComboButtonStyle>(FPinRestyleStyles::ComboButton).
					                                                      ButtonStyle
				...
				// Padding is in ButtonStyle
				                                                  ...
				                                                  .OnClicked_Lambda([ValueWidget]() -> FReply
				                                                  {
					                                                  ...
					                                                  auto _ComboButton = GetChild<SComboButton, 0>(_HorizontalBox);
					                                                  auto& _Children = access_private::Children(_ComboButton.Get());
					                                                  auto _ComboButton_Button = StaticCastSharedRef<SButton>(
						                                                  _Children.GetChildAt(0));
					                                                  auto _ComboButton_Button_HorizontalBox = GetChild<SHorizontalBox>(
						                                                  _ComboButton_Button);
...
bool SDefault_PoseWatchOverlay::IsPoseWatchValid() const { return PoseWatch.IsValid(); }
