...
FDefaultAnimGraphConnectionDrawingPolicy::FDefaultAnimGraphConnectionDrawingPolicy(int32 InBackLayerID, int32 InFrontLayerID, float ZoomFactor,
...
bool FDefaultAnimGraphConnectionDrawingPolicy::TreatWireAsExecutionPin(UEdGraphPin* InputPin, UEdGraphPin* OutputPin) const
{
	...
	return (InputPin != nullptr) && (Schema->IsPosePin(OutputPin->PinType));
}
...
void FDefaultAnimGraphConnectionDrawingPolicy::BuildExecutionRoadmap()
{
	if (UAnimBlueprint* TargetBP = Cast<UAnimBlueprint>(FBlueprintEditorUtils::FindBlueprintForGraph(GraphObj))) {
		UAnimBlueprintGeneratedClass* AnimBlueprintClass = static_cast<UAnimBlueprintGeneratedClass*>(*(TargetBP->GeneratedClass));

		if (TargetBP->GetObjectBeingDebugged() == nullptr) { return; }
		...
		for (auto VisitIt = DebugInfo.UpdatedNodesThisFrame.CreateIterator(); VisitIt; ++VisitIt) {
			...
			if ((VisitRecord.SourceID >= 0) && (VisitRecord.SourceID < NumAnimNodeProperties) && (VisitRecord.TargetID >= 0) && (VisitRecord.
				TargetID < NumAnimNodeProperties)) {
				...
				if (const UAnimGraphNode_Base* SourceNode = Cast<const
					UAnimGraphNode_Base>(DebugInfo.NodePropertyIndexToNodeMap.FindRef(ReverseSourceID))) {
					if (const UAnimGraphNode_Base* TargetNode = Cast<const UAnimGraphNode_Base>(
						DebugInfo.NodePropertyIndexToNodeMap.FindRef(ReverseTargetID))) {
	...
	if (WireParams.AssociatedPin1 && WireParams.AssociatedPin2) {
		if (UAnimationGraphSchema::IsPosePin(WireParams.AssociatedPin1->PinType)) {
			...
			if (UsePin1 && UsePin2) {
				...
				if (Node1 && Node2) {
					const TArrayView<const SDefault_GraphPinPose::FAttributeInfo>& AdditionalAttributeInfo = PinAttributes.FindRef(
						WireParams.AssociatedPin1);
					...
					if (AdditionalAttributeInfo.Num() > 0) {
						...
						_WireParams.WireColor = UWireRestyleSettings::Get()->AttributeWireColor.Get();
						FLinearColor(1.0, 0.5, 0.0, 1.0);
...
void FDefaultAnimGraphConnectionDrawingPolicy::BuildPinToPinWidgetMap(TMap<TSharedRef<SWidget>, FArrangedWidget>& InPinGeometries)
{
	...
	for (const TPair<UEdGraphPin*, TSharedPtr<SGraphPin>>& PinWidgetPair : PinToPinWidgetMap) {
		if (PinWidgetPair.Key->Direction == EGPD_Output && UAnimationGraphSchema::IsPosePin(PinWidgetPair.Key->PinType) && PinWidgetPair.Key->
			GetOwningNode()->IsA<UAnimGraphNode_Base>()) {
...
void FDefaultAnimGraphConnectionDrawingPolicy::DetermineStyleOfExecWire(float& Thickness, FLinearColor& WireColor, bool& bDrawBubbles,
...
void FDefaultAnimGraphConnectionDrawingPolicy::ApplyHoverDeemphasis(UEdGraphPin* OutputPin, UEdGraphPin* InputPin, float& Thickness,
                                                                    FLinearColor& WireColor)
{
	bool bDebugging = Thickness > ReleaseWireThickness;
	if (!bDebugging) { FDefaultConnectionDrawingPolicy::ApplyHoverDeemphasis(OutputPin, InputPin, Thickness, WireColor); }
}
