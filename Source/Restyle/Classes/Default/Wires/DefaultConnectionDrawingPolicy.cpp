...
class FPathDrawerHolder
{
	...
	public:
		...
		virtual void Stop() override { bUpdate = false; }

		virtual void Exit() override { bUpdate = false; }

		virtual FSingleThreadRunnable* GetSingleThreadInterface() override { return nullptr; }

		virtual ~FRunnableCleaner() override { bUpdate = false; }
...
private:
	...
	void RemoveGC() { if (CleanerThread) CleanerThread->Kill(false); }

	FPathDrawerHolder()
		: TimerHandle(FTimerHandle())
		  , TimerDelegate(FTimerDelegate()) { SetupGC(); }
                                                        ...
                                                        const FConnectionParams& Params, float ZoomValue)
{
	if (Settings->bTreatSplinesLikePins) {
		...
		else if (ClosestDistanceSquared < QueryDistanceForCloseSquared) { SplineOverlapResult.SetCloseToSpline(true); }
	}
}
...
uint32 FDefaultConnectionDrawingPolicy::GetPinId(SGraphPin* PinWidget)
{
	...
	for (int i = 0; i < PinsNum; ++i) {
		...
		if (This->GetDirection() == EGPD_Input) { LastInputPinId = i; }

		if (This == PinWidget) { return PinDirection == EGPD_Output ? (i - LastInputPinId - 1) : i; }
	}
	return 0;
}
                                                            ...
                                                            TArray<FVector2f>* InPoints)
{
		...
		if (WireSettings->MinHorizontalLength > WireParams.WireThickness) {
			...
			if (!Thrown++) {
				UE_LOG(LogTemp, Error,
				       TEXT(
					       "[Restyle] Wire->MinHorizontalLength must be greater than wire thickness. otherwise line's geometry will be messed up"
				       ));
			}
		}
	}
	...
	if (WireParams.bDrawBubbles || (MidpointImage != nullptr)) {
		...
		if (WireParams.bDrawBubbles) { DrawBubbles(Points, TotalLength, WireParams); }
		if (MidpointImage) { DrawMidpointImage(Points, TotalLength, WireParams); }
	}
}
...
void FDefaultConnectionDrawingPolicy::DrawPath(TArray<FVector2f>& Points, const FConnectionParams& WireParams)
{
	...
	const UWireRestyleSettings* WireSettings = UWireRestyleSettings::Get();
	FLinearColor WireColor = WireParams.WireColor;
	FColor CorrectedWireColor = WireParams.WireColor.ToFColorSRGB();
	WireColor = {
			CorrectedWireColor.R / 255.f,
	...
	// Convert points to FVector2D for standard Slate drawing
	TArray<FVector2D> PointsD;
	for (auto& it : Points) { PointsD.Add(FVector2D(it.X, it.Y)); }
	...
	// Draw using standard Slate lines instead of custom element
	...
	// If you still want to keep the custom element logic (but have the standard as fallback):
	auto LineDrawer = FPathDrawerHolder::Get().Add();
	LineDrawer->ClippingRect = ClippingRect;
	LineDrawer->Path.Init(Points, PathSettings);
	Points = LineDrawer->Path.Points;
	...
	const FRestyleConnectionParams& Params, const FConnectionParams& WireParams)
{
	 ...
	 * [a]---[b]
	 */
	if (bIs0Deg) { PointsF.Append({Start, End}); }
	/*
		 [a]--.
		 .___|
		 |__[b]
	 */
		...
		if (WireSettings->BackwardConnectionPriority == EWireRestylePriority::Output) {
			if (bStartIsHigher) { SnapPointY = StartNode.AbsolutePosition.Y + Zoomed(StartNode.Size.Y) + Offset; }
			else { SnapPointY = StartNode.AbsolutePosition.Y - Offset; }
		}
		else if (WireSettings->BackwardConnectionPriority == EWireRestylePriority::Input) {
			if (bStartIsHigher) { SnapPointY = EndNode.AbsolutePosition.Y - Offset; }
			else { SnapPointY = EndNode.AbsolutePosition.Y + Zoomed(EndNode.Size.Y) + Offset; }
		}
		else {
			if (bStartIsHigher) {
	 ...
	 */
		...
		if (bApplyAntiCollision) {
			...
			if (Params.NumInputPins == 1 && !bIsPin1Knot) { PinPriority = EWireRestylePriority::Output; }
			else if (Params.NumOutputPins == 1 && !bIsPin2Knot) { PinPriority = EWireRestylePriority::Input; }

			float StepMultiplier = ApplyTriWave(
				PinPriority == EWireRestylePriority::Input
					? Params.InPinId
					: Params.OutPinId, Period);
	 ...
	 */
		...
		if (b45DegreeStyle) {
			...
			if (bApplyAntiCollision) {
				...
				if (Params.NumInputPins == 1) { PinPriority = EWireRestylePriority::Output; }
				else if (Params.NumOutputPins == 1) { PinPriority = EWireRestylePriority::Input; }
				...
				else if (bBehindOutput) { StartEx.X = EndEx.X; }
				else if (bAfterInput) { EndEx.X = StartEx.X; }
			}
		}
		else {
			if (TransitionPriority == EWireRestylePriority::Output) {
                                                           ...
                                                           float& Thickness, FLinearColor& WireColor)
{
	...
	const float FadeInPeriod = UWireRestyleSettings::Get()->HoverFadeInPeriod;
	...
	const float TimeFraction = FMath::SmoothStep(0.0f, FadeInPeriod,
	                                             static_cast<float>(FSlateApplication::Get().GetCurrentTime() - LastHoverTimeEvent -
		                                             FadeInBias));
	...
	else { WireColor = FMath::Lerp<FLinearColor>(WireColor, DarkenedColor, DarkFraction * TimeFraction); }
}
