...
FName FRestyleDefaultTheme::GetId() const { return TEXT("Default"); }
...
{
	Update();
	if (NodeFactoryProvider->IsRegistered()) { NodeFactoryProvider->Update(); }
	if (PinFactoryProvider->IsRegistered()) { PinFactoryProvider->Update(); }
