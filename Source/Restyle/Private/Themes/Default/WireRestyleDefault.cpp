...
FWireRestyleDefault::FWireRestyleDefault() { Factory = MakeShared<FDefaultGraphPanelWireFactory>(); }

ERestyleSubject FWireRestyleDefault::GetSubject() { return ERestyleSubject::PinConnection; }
...
TSharedPtr<FGraphPanelPinConnectionFactory> FWireRestyleDefault::GetFactory() { return Factory; }
...
bool FWireRestyleDefault::IsRegistered() { return bRegistered; }
...
void UWireRestyleSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.GetPropertyName() ==
		...
		GET_MEMBER_NAME_CHECKED(UWireRestyleSettings, EdgeOffsetPeriod)) { UpdateAntiCollisionLevels(); }
}
